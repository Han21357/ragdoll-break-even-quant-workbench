...
仓库包含 `render.yaml` 与 `wsgi.py`，可直接部署完整 Flask 平台。公开实例应设置 `RAGDOLL_PUBLIC_DEMO=1`：系统会使用隔离数据库、跳过本地历史数据迁移并禁用私人 LLM 密钥。免费实例的 SQLite 数据可能在重启或重新部署后重置，因此只适合作品演示，不适合作为真实持仓账本。
